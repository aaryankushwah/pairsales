...
export async function execute(ctx: AdapterExecutionContext): Promise<AdapterExecutionResult> {
  ...
  // Prefer adapterConfig.sessionKey; fall back to agent:<slug>:main
  const heartbeatSessionKey = (() => {
    const configured = nonEmpty(ctx.config.sessionKey);
    if (configured) return configured;
    const slug =
      nonEmpty(ctx.agent.name)
        ?.toLowerCase()
        .replace(/[^a-z0-9]+/g, "-")
        .replace(/^-|-$/g, "") ?? ctx.agent.id;
    return `agent:${slug}:main`;
  })();

  // Try to read the agent's HEARTBEAT.md to use as the prompt.
  // Derive the agent slug from sessionKey (agent:<slug>:main → <slug>).
  const heartbeatPrompt = await (async () => {
    const slugMatch = heartbeatSessionKey.match(/^agent:([^:]+):main$/);
    if (!slugMatch) return null;
    const slug = slugMatch[1];
    // Look for HEARTBEAT.md relative to process cwd (the app root)
    const candidates = [
      path.resolve(process.cwd(), "agents", slug, "HEARTBEAT.md"),
      path.resolve(process.cwd(), "agents", slug, "heartbeat.md"),
    ];
  ...
  // Use HEARTBEAT.md as primary prompt if available; otherwise fall back to wake text
  const heartbeatMessage = heartbeatPrompt
    ? `${heartbeatPrompt}\n\n---\n\n${message}`
    : message;

  await ctx.onLog(
    "stdout",
    `[openclaw-gateway] connecting via GatewayClient to ${parsedUrl.toString()} session=${heartbeatSessionKey}\n`,
  );
  if (heartbeatPrompt) {
    await ctx.onLog(
      "stdout",
      `[openclaw-gateway] loaded HEARTBEAT.md (${heartbeatPrompt.length} chars)\n`,
    );
  }
  ...
  try {
    ...
    await ctx.onLog("stdout", "[openclaw-gateway] connected\n");

    await gatewayClient.ensureSession(heartbeatSessionKey);

    const responseText = await gatewayClient.sendMessage(heartbeatSessionKey, heartbeatMessage);

    await ctx.onLog(
      "stdout",
      `[openclaw-gateway] run completed session=${heartbeatSessionKey} status=ok\n`,
    );
    ...
    const timedOut = errMessage.toLowerCase().includes("timeout");

    await ctx.onLog("stderr", `[openclaw-gateway] request failed: ${errMessage}\n`);
